...
namespace LogViewer {
	...
	public class MemMapFile : IDisposable {
		...
		[DllImport("kernel32.dll", SetLastError = true)]
		static extern IntPtr MapViewOfFileEx(
		...
		public MemMapView AllocateView(ulong position, ulong byteCount) {
			...
			unsafe {
				//	指定された位置をメモリにマッピング
				IntPtr viewAddress = MapViewOfFileEx(m_MappingHandle, m_FileMapAccess, (uint)(mapPosition >> 32), (uint)(mapPosition & 0xffffffff), new UIntPtr((ulong)byteCount + (ulong)offset), IntPtr.Zero);
	...
	/// <summary>
	/// MemMapFile 用のアクセス例外クラス。
	/// </summary>
	public class MemMapFileAccessException : ApplicationException {
		private string m_FileName;

		/// <summary>
		/// コンストラクタ。
		/// </summary>
		public MemMapFileAccessException(string msg, string fileName)
			: base(string.Format("{0} (ファイル名 \"{1}\")", msg, fileName)) {
			m_FileName = fileName;
		}
		...
		/// コンストラクタ。
		/// </summary>
		public MemMapFileAccessException(int ecode, string fileName)
			: base(string.Format("{0} (ファイル名 \"{1}\")", (new System.ComponentModel.Win32Exception(ecode)).Message, fileName)) {
			m_FileName = fileName;
		}
